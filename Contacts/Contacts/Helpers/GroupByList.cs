...
{
    public class GroupByList<T> : IReadOnlyCollection<KeyValuePair<string, List<T>>>
        where T : IGroupByList
    {
        ...
        public GroupByList(string alphabet)
        {
            _alphabet = alphabet;
        ...
        private void AddItem(string keyName, T item)
        {
            ...
            if (isExist)
            {
                ...
                _groupedList[index].Value.Sort(new ListComparer<T>());
            }
            else
            {
                _groupedList.Add(new KeyValuePair<string, List<T>>(key, new List<T> { item }));
                _groupedList.Sort(new KeyListComparer<T>());
            }
        }
    }
}
